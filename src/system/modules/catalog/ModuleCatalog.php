...
abstract class ModuleCatalog extends Module
{
	...
	protected function buildTagsQuery(array $arrFieldConf, array $arrTags)
	{
		...
		if (count($arrTags) == 0)
			$result = 'false'; // yes, sql statement false
		...
		else
		{
			...
			if($this->catalog_tags_mode == 'AND')
			{
				...
				foreach ($arrTags as $i => $intValueId) {
					$joins .= sprintf(' INNER JOIN tl_catalog_tag_rel r%u
				  											ON r%u.fieldid=%u AND r%u.valueid=%u AND r%u.itemid=r%u.itemid',
														$i+1, $i+1, $fieldId, $i+1, $intValueId, $i+1, $i);
				}
				
				$result = sprintf('id IN (SELECT DISTINCT(r0.itemid) AS id FROM tl_catalog_tag_rel r0 %s)',
													$joins);
			} else {
